...
/obj/machinery/door/airlock/glass_large
	...
	doortype = 10
	transparent = 1
	var/obj/machinery/door/airlock/glass_large/bump/connected = null
...
/obj/machinery/door/airlock/glass_large/bump
	...
	New()
		src.connected = locate(/obj/machinery/door/airlock/glass_large, get_step(loc, WEST))
	...
	switch(wireIndex)
		...
		if (AIRLOCK_WIRE_DOOR_BOLTS)
			...
			if (!src.locked)
				...
				playsound(src.loc, 'bolts.ogg', 5, 0)
				src.updateUsrDialog()
			else
...
/obj/machinery/door/airlock/animater(animation)
	switch(animation)
		if("opening")
			...
			if(is_hot || is_cold)
				if(is_hot)
					usr << "\red The airlock feels very warm! Are you sure you want to open it?"
				else if(is_cold)
					usr << "\red The airlock feels very cold! Are you sure you want to open it?"
				if(!clickedon)
					clickedon = 1
					return
			HOT AND COLD CHECKS
...
/obj/machinery/door/airlock/proc/hack(mob/user as mob)
	if (src.aiHacking==0)
		...
		spawn(20)
			...
			user << "Airlock remote control has been blocked. Beginning fault-detection."
			sleep(50)
			if (src.canAIControl())
...
/obj/machinery/door/airlock/attack_hand(mob/user as mob)
	...
	if (ishuman(user) && prob(40) && src.density)
		...
		if(H.brainloss >= 60)
			src.health = max(0, src.health - 15)
			playsound(src.loc, 'bang.ogg', 25, 1)
			if(!istype(H.head, /obj/item/clothing/head/helmet))
				for(var/mob/M in viewers(src, null))
					M << "\red [user] smashes the airlock with their head!"
				var/datum/organ/external/affecting = H.organs["head"]
				affecting.take_damage(10, 0)
				if(H.mutantrace == "zombie")
					H.stunned = 0
					H.weakened = 0
				else
					H.stunned = 8
				...
				for(var/mob/M in viewers(src, null))
					M << "\red [user] smashes the airlock with their helmet!."
			return
...
/obj/machinery/door/airlock/open()
	if (src.welded || src.locked || (!src.arePowerSystemsOn()) || (stat & NOPOWER) || src.isWireCut(AIRLOCK_WIRE_OPEN_DOOR))
		return 0
	use_power(50)
	playsound(src.loc, 'airlock_up.ogg', rand(20,30), 1)
...
/obj/machinery/door/airlock/close()
	if (src.welded || src.locked || (!src.arePowerSystemsOn()) || (stat & NOPOWER) || src.isWireCut(AIRLOCK_WIRE_DOOR_BOLTS))
		return
	use_power(50)
	playsound(src.loc, 'airlock_down.ogg', rand(20,30), 1)
	..()
	return
