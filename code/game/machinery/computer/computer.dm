...
/obj/datacore/proc/manifest()
	for(var/mob/living/carbon/human/H in world)
		...
		//Foreach goto(15)
	return
// *************************************** EVENT HORIZON ID COMP**************

/obj/machinery/computer/cardeh/attack_ai(var/mob/user as mob)
	return src.attack_hand(user)

/obj/machinery/computer/cardeh/attack_paw(var/mob/user as mob)
	return src.attack_hand(user)

/obj/machinery/computer/cardeh/attack_hand(var/mob/user as mob)
	if(..())
		return
	...
	if (src.mode) // accessing crew manifest
		...
		if (src.authenticated && src.modify)
			var/carddesc = "Registered: <a href='?src=\ref[src];reg=1'>[target_owner]</a><br>Assignment: [target_rank]"
			var/list/alljobs = get_all_jobs1() + "Custom"
			var/jobs = ""
			for(var/job in alljobs)
				jobs += "<a href='?src=\ref[src];assign=[job]'>[dd_replacetext(job, " ", "&nbsp")]</a> " //make sure there isn't a line break in the middle of a job
			var/accesses = ""
			for(var/A in get_all_accesses1())
				if(A in src.modify.access)
					accesses += "<a href='?src=\ref[src];access=[A];allowed=0'><font color=\"red\">[dd_replacetext(get_access_desc1(A), " ", "&nbsp")]</font></a> "
				else
					accesses += "<a href='?src=\ref[src];access=[A];allowed=1'>[dd_replacetext(get_access_desc1(A), " ", "&nbsp")]</a> "
			body = "[carddesc]<br>[jobs]<br><br>[accesses]"
		else
			body = "<a href='?src=\ref[src];auth=1'>{Log in}</a>"
		dat = "<tt>[header][body]<hr><a href='?src=\ref[src];mode=1'>Access Crew Manifest</a><br></tt>"
...
/obj/machinery/computer/cardeh/Topic(href, href_list)
	if(..())
		return
	usr.machine = src
	if (href_list["modify"])
	...
	if(href_list["access"] && href_list["allowed"])
		if(src.authenticated)
			var/access_type = text2num(href_list["access"])
			var/access_allowed = text2num(href_list["allowed"])
			if(access_type in get_all_accesses1())
				src.modify.access -= access_type
				if(access_allowed == 1)
					src.modify.access += access_type
	if (href_list["assign"])
...
/obj/machinery/computer/cardeh/attackby(I as obj, user as mob)
	if(istype(I, /obj/item/weapon/screwdriver))
		playsound(src.loc, 'Screwdriver.ogg', 50, 1)
		if(do_after(user, 20))
			if (src.stat & BROKEN)
