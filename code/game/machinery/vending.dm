...
/obj/machinery/vending/attack_hand(mob/user as mob)
	...
	if (src.product_cost != 0)
		dat = text("<b>Insert Money:</b> <A href='?src=\ref[];auth=1'>[]</A><HR>", src, (src.auth ? "(money inserted)" : "(insert)"))
		/*dat += "<b>Insert Money:</b> <A href='?src=\ref[];auth=1'>"
		if(src.auth == "1")
			dat += "++++++++++"
		else
			dat += "----------"
	...
	if (src.product_records.len == 0)
		...
		for (var/datum/data/vending_product/R in display_records)
			...
			if (R.amount > 0)
				...
				if (src.product_cost != 0)
					dat += " <small>(Costs [src.product_cost] D2S)</small>"
			else
		...
		if (href_list["auth"])
			if (src.auth)
				...
				if (istype(I, /obj/item/weapon/spacecash))
					if (I.moneyvalue == src.product_cost)
						usr.drop_item()
						I.loc = src
						src.auth = 1
					else
						usr << "\red You need to insert the correct amount of money!"
						usr << "\red (click on the money in your hand and type in the amount required by this machine, then pick it up and insert it.)"
