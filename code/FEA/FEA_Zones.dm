...
var
	FLOW_PERCENT = 10 //Percent of gas to send between connected turfs.
	VACUUM_SPEED = 5.2 //Divisor of zone gases exposed directly to space (i.e. space tiles in members)

#define QUANTIZE(variable)		(round(variable,0.001))

var/list/zones = list()
var/zone_update_delay = 8
var/stop_zones = 0
...
zone
	...
	Update()
		while(1)
			...
			for(var/datum/gas_mixture/virus_holder in members)
				for(var/datum/disease/D in viruses)
					for(var/datum/disease/C in viruses)
						if(istype(D, C.type))
							viruses -= D
			...
			oxygen_archive = round(oxygen_archive ,0.01)
			nitrogen_archive = round(nitrogen_archive ,0.01)
			co2_archive = round(co2_archive ,0.01)
			...
			if(space_connections.len)				 //Throw gas into space if it has space connections.
				...
				for(var/turf/simulated/M in members)
					...
					GM.remove_ratio(1/VACUUM_SPEED)
			...
			if(!disable_connections)
				...
				for(var/zone/Z in connections)
					...
					viruses = list()
					for(var/datum/gas_mixture/virus_holder in members)
						if(virus_holder.viruses.len > 0)
							for(var/datum/disease/D in virus_holder.viruses)
								for(var/datum/disease/C in viruses)
									if(!istype(D, C.type))
										viruses += D
						//End Magic
	...
	oxygen(n)
		if(!n) return round(turf_oxy, 0.01)
		else
			turf_oxy = n
			oxygen = round(turf_oxy*members.len, 0.01)
	nitrogen(n)
		if(!n) return round(turf_nitro, 0.01)
		else
			turf_nitro = n
			nitrogen = round(turf_nitro*members.len, 0.01)
	co2(n)
		if(!n) return round(turf_co2, 0.01)
		else
			turf_co2 = n
			co2 = round(turf_co2*members.len, 0.01)
...
/*
mob/verb/Zone_Info(turf/T as null|turf)
	set category = "Debug"
	if(T)
		T.ZoneInfo()
	else
