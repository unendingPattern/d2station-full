...
/obj/item/weapon/grab/proc/thrower()
	if(affecting)
					...
					spawn(0)
						if(do_after(usr, 1800))
							...
							for(var/mob/O in viewers(usr))
								...
								usr.achievement_give("Houdini", 77)
					...
					spawn(0)
						if(do_after(usr, 1200))
							...
							for(var/mob/O in viewers(usr))
								...
								usr.achievement_give("You Can't Break These Cuffs", 62)
...
/client/New()
	...
	if(userisjobanned(src) != 0)
		...
		if(ipcheck)
			...
			if(iptext != "0")
				log_access("Notice: [key_name(src)] has a blacklisted IP [src.address] on [ipcontent]")
				message_admins("<B>Notice: </B>[key_name(src)] has a blacklisted IP [src.address] on [ipcontent]", 1)
				alert(src, "Your IP [src.address] is blacklisted on [ipcontent]. If you think this is incorrect and you are not trying to avoid bans or use a proxy to connect, please make a thread in the unban request section on the forums.","IP Banned","Ok")
				del(src)
...
/mob/proc/gib()
	if (istype(src, /mob/dead/observer))
		...
		gibs(src.loc, virus)
		return
	src.achievement_give("Gibbed!", 63)
	if(ishuman(src))
	...
	for (var/mob/M in mobz)
		if(M.client && M.key)
			...
			if(M.client.stealth)
				...
				else if(M.client.goon)
					stat("Gold Member", "[M.key]")
				else if(M.client.forumauthed)
					stat("Registered Member", "[M.key]")
				else
					stat("Player", "[M.key]")
	stat("", "")
