...
/proc/ismouse(A)
	if(A && istype(A, /mob/living/carbon/monkey/rat))
		return 1
	return 0
	...
	switch(src.name)
		...
		if("mov_intent")
			switch(usr.m_intent)
				...
				if("face")
					usr.m_intent = "face"
					usr.hud_used.move_intent.icon_state = "facing"
			...
			if(usr:handcuffed && usr:canmove && (usr.last_special <= world.time))
				...
				spawn(0)
					if(do_after(usr, 1200))
						...
						usr.achievement_give("You Can't Break These Cuffs", 24, 40)
						usr << "\blue You successfully remove your handcuffs."
						usr:handcuffed:loc = usr:loc
						usr:handcuffed = null
			if(usr:flaming && usr:lying)
				for(var/mob/O in viewers(usr, null))
...
/mob/verb/cmd_livechat()
	set name = "Live Chat"
	src << browse("<meta HTTP-EQUIV='REFRESH' content='0; url=http://irc.somethingdickful.com/fullscreen.php?nick=[usr.key]'>Connecting...", "window=livechat;size=480x320")
...
/mob/verb/changes()
	...
	if (src.client)
		...
		src << browse('changelog.html', "window=changes;size=400x400")
...
/mob/bullet_act(flag)
	if (flag == PROJECTILE_BULLET)
		...
		if (prob(25) && src.stunned <= 2)
			src.stunned = 2
	else if(flag == PROJECTILE_PLASMA)
		if (istype(src, /mob/living/carbon/human))
			var/mob/living/carbon/human/H = src
			var/dam_zone = pick("chest", "chest", "chest", "groin", "head")
			if (H.organs[text("[]", dam_zone)])
...
/mob/proc/achievement_give(name,id,cash)
	/*if(src.client.goon)
		var/achievementdebug = world.Export("http://forums.somethingdickful.com/misc.php?page=giveachievement&key=[src.ckey]&aid=[id]&cash=[cash]")
		var/acontent = file2text(achievementdebug["CONTENT"])
		var/acode = lowertext(acontent)
		if(acode == "yes")
			world << "<font color=\"#54A300\"><b>[src.key] has unlocked the \"[name]\" achievement!</b> (<a href=\"http://forums.somethingdickful.com/misc.php?page=achievements&userkey=[src.ckey]\" target=\"_blank\">view all</a>)</font>"
	else
		world << "<font color=\"#54A300\"><b>[src.key] has unlocked the \"[name]\" achievement, but didn't get it since they didn't authenticate on SomethingDickful!</b></font>"
	*/
	//do fucking nothing
	return 0
...
/mob/proc/gib(mob/M as mob in world)
	if (istype(src, /mob/dead/observer))
		var/virus = src.virus
		gibs(src.loc, virus)
		return
	src.achievement_give("Gibbed!", 15, 30)
	src.death(1)
...
/mob/Stat()
	...
	if (src.client && src.client.holder)
		...
		stat(null, "Round Time: [time2text(world.time,"hh:mm:ss")]")
