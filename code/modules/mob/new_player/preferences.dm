...
*/
	...
	proc/SetChoices(mob/user, occ=1)
		if(IsGuestKey(user.key))
			alert("Guest accounts are restricted to Tourist. Log in with a BYOND account to unlock the station jobs!")
			return
		var/HTML = "<body>"
		HTML += "<tt><center>"

//		if(!IsGuestKey(user.key))
//			HTML += "<font color=\"red\"><b>IF YOU WANT TO PLAY AS SOME OF THE JOBS YOU MUST GET WHITELISTED, YOU CAN DO THIS <a href=\"http://forums.somethingdickful.com/forumdisplay.php?fid=42\" target=\"_blank\">RIGHT HERE</a></b></font><br>"
//			HTML += "<b>THESE JOBS ARE:</b> Captain, AI, Head of Personnel, Head of Security, Security, Chief Engineer, Research Director, Warden, Detective, Chief Medical Officer"
//			HTML += "<br><br>"
	...
	proc/process_link(mob/user, list/link_tags)
		...
		if (link_tags["age"])
			switch(link_tags["age"])
				if ("input")
					var/new_age = input(user, "Please select type in age: 18-45", "Character Generation")  as num
					if(new_age)
						src.age = max(min(round(text2num(new_age)), 45), 18)
				if ("random")
					src.age = rand (18, 45)
			...
			switch(link_tags["h_style"])
				if ("random")
					if (src.gender == FEMALE)
						src.h_style = pickweight ( list ("Cut Hair" = 5, "Short Hair" = 5, "Long Hair" = 5, "Mohawk" = 5, "Balding" = 1, "Fag" = 5, "Afro" = 5, "Bieber" = 5, "Bedhead" = 5, "Dreadlocks" = 5, "Rocker" = 5, "Bald" = 5, "Super-long Hair" = 5, "Ponytail" = 5, "Short Female Hair" = 5, "Scene Hair" = 5, "Girly Hair" = 5, "Messy Hair" = 5))
					else
						src.h_style = pickweight ( list ("Cut Hair" = 5, "Short Hair" = 5, "Long Hair" = 5, "Mohawk" = 5, "Balding" = 5, "Fag" = 5, "Afro" = 5, "Bieber" = 5, "Bedhead" = 5, "Dreadlocks" = 5, "Rocker" = 5, "Bald" = 5, "Super-long Hair" = 5, "Ponytail" = 5, "Short Female Hair" = 5, "Scene Hair" = 5, "Girly Hair" = 5, "Messy Hair" = 5))

				if("input")
					var/new_style = input(user, "Please select hair style", "Character Generation")  as null|anything in list( "Cut Hair", "Short Hair", "Long Hair", "Mohawk", "Balding", "Fag", "Afro", "Bieber", "Bedhead", "Dreadlocks", "Rocker", "Bald", "Super-long Hair", "Ponytail", "Short Female Hair", "Scene Hair", "Girly Hair", "Messy Hair" )
					if (new_style)
