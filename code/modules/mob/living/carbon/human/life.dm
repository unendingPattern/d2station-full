...
/mob/living/carbon/human/Life()
	...
	// Handling poop
	if(src.r_hand)
		if((istype(src:r_hand, /obj/item/weapon/reagent_containers/food/snacks/poo)) || (istype(src:l_hand, /obj/item/weapon/reagent_containers/food/snacks/poo)))
			if(!src.gloves)
				src.pickeduppoo = 1
			else
				src.gloves.add_poo()
	...
	proc
		...
		handle_chemicals_in_body()
			...
			if(overeatduration > 500 && !(src.mutations & 32))
				...
				src.mutations |= 32
				update_body()
			if (overeatduration < 300 && src.mutations & 32)
		...
		handle_regular_status_updates()
			...
			/*if(src.horny)
				if(prob(1))
					if(prob(6))
						emote(pick("blush","smile"))
				if(prob(1))
					if(prob(2))
						emote(pick("moan","wink"))
				if(prob(1))
					if(prob(10))
						src.horny = 0*/
		...
		handle_regular_hud_updates()
			...
			if (src.stat == 2 || src.mutations & 4)
				...
				if(!src.druggy)
					src.see_invisible = 2
			else if (istype(src.wear_mask, /obj/item/clothing/mask/gas))
			...
			if (!istype(src.glasses, /obj/item/clothing/glasses/thermal))
				...
				if(client)
					...
					for(var/mob/living/carbon/human/patient in view(src))
						...
						var/foundVirus = 0
						for(var/datum/disease/D in patient.virus)
							foundVirus++
				...
				if(client)
					...
					for(var/mob/living/carbon/human/perp in view(src))
						if(perp.wear_id)
							var/I = image('hud.dmi',perp, icon_state = "hud[ckey(perp:wear_id:GetJobName())]")
							src.client.images += I
							var/perpname = "wot"
								...
								if (E.fields["name"] == perpname)
									for (var/datum/data/record/R in data_core.security)
										if ((R.fields["id"] == E.fields["id"]) && (R.fields["criminal"] == "*Arrest*"))
											var/L = image('hud.dmi',perp, icon_state = "hudwanted")
											src.client.images += L
											break
										else if((R.fields["id"] == E.fields["id"]) && (R.fields["criminal"] == "Incarcerated"))
											var/N = image('hud.dmi',perp, icon_state = "hudprisoner")
											src.client.images += N
											break
						else
							client.images += image('hud.dmi',perp,"hudunknown")
						for(var/obj/item/weapon/implant/tracking/tracker in perp)
							if(tracker.implanted)
								var/I = image('hud.dmi',perp, icon_state = "hudtracking")
								src.client.images += I
								break
				if (!druggy)
					see_invisible = 0
			if ((src.blind && src.stat != 2))
				if ((src.blinded))
					if (!istype(src.glasses, /obj/item/clothing/glasses/opticlink))
						src.blind.layer = 18
				else
					src.blind.layer = 0
		...
		handle_paydays()
			if(src.stat < 2)
				if(!paytime)
					paytime = world.time
				else if(world.time - paytime > 9000)
					if (istype(src.wear_id, /obj/item/weapon/card/id))
						var/paidtoday = rand(10, 20)
						src << "\red Payday! You got [paidtoday] space euros!"
						src.wear_id.credit += paidtoday
					paytime = 0
				return
			...
			if(src.stat < 2)
				if(!nicsmoketime)
					...
					if(src.nicotineaddiction == 1)
						src << "\red You really want a smoke!"
						src.health -= 5
						src.toxloss += 5
						emote("cough")
						src.updatehealth()
