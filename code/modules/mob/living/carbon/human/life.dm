...
/mob/living/carbon/human/Life()
	...
	if (stat != 2) // if not dead, dont call these specific procs
		...
		//This here is a drug check, please step out of the car sir.
		handle_drugs()
...
*/
		...
		//More effects and more drugs coming soon.
		handle_drugs()
			if(reagents.has_reagent("thc"))
				if(stoned <= 15) //Loosely translated; if stoned = baby's first high...
					druggy = max(druggy, 20)
					if(prob(10)) emote("smile")
				else if(stoned <= 30) //This is about as sober as I ever get.
					druggy = max(druggy, 30)
					if(prob(10)) emote(pick("giggle","smile"))
				else if(stoned <= 45) //Slowly building momentum.
					druggy = max(druggy, 40)
					make_dizzy(1)
					if(prob(20)) emote(pick("giggle","smile","laugh"))
				else if(stoned <= 60) //Power of the hour. We're certainly sky surfing at this point. For those unfamiliar with weed; client = pretty high guy.
					druggy = max(druggy, 60)
					make_dizzy(2)
					if(prob(20)) emote(pick("giggle","laugh","smile"))
				else if(stoned <= 90) //Now we're high. Really high.
					kanye = max(kanye, 80)
					druggy = max(druggy, 80)
					make_dizzy(5)
					if(prob(30)) emote(pick("giggle","laugh","smile"))
					if(prob(1)) emote("fart") //It's not that getting high makes you fart, it's just that at this stage, you simply do not care to hold it in.
				else if(stoned <= 120) //Still high
					kanye = max(kanye, 100)
					druggy = max(druggy, 100)
					make_dizzy(5)
					if(prob(45)) emote(pick("giggle","laugh"))
					if(prob(1)) emote("fart")
				else if(stoned <= 150) //Really high
					kanye = max(kanye, 140)
				...
				else if(stoned <= 200) //Very god damn high.
					kanye = max(kanye, 160)
					...
					if(prob(2) )emote("fart")
				else if(stoned <= 255) //Ultra fucking high.
					kanye = max(kanye, 200)
					druggy = max(druggy, 200)
					make_dizzy(20)
					if(prob(65)) emote(pick("giggle","laugh"))
					if(prob(2)) emote("fart")
				else if (stoned <= 420) //God tier. Getting this high should get you a medal.
					kanye = max(kanye, 250)
				...
				else if (stoned >= 420) //MARIJUANA CHAMPION TIER. CONGRATULATIONS, NOW BLAZE IT FAGGOT. You may get turned into Snoop Doglion or Kanye West. Or, a super nigger.
			...
			if(reagents.has_reagent("psilocybin"))
				if(tripping <= 10) //Baby's first trip
					...
					if(prob(1)) //It's all in your head.
					...
					druggy = max(druggy, 140)
					if(prob(10)) say(pick(";MURRRRRR", "BLERG", "mmmmmmmRRRRRRRRRRRRRNNNNNNNNNNNNNNNNNN", "Dudes check me out I'm a motorbike VROOOM VROOOM CRASH DEAD", ";AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARGH!!!!", ";No. No!! NOOOOOOOOOOOOOOOOOOOOOOOOOOOO!", ";Murglefurgle, the shizniz be gammon back to the fucking... fucking... what?", ";Guys, can someone please take me to medbay? Or disposals?", ";THUNDERCUNTS!", ";THUNDERCUNTS EVERYWHERE!!", ";Moo.", "Moo", ";MOOOOOOOOOOOO!", "Meow.", "Woof. WOOF WOOF WOOF GET OUTTA MY TERRITORY", ";I wish I could bring you more pertinent news, but we're in a rating's system, and the key factor is SENSATIONALISM", ";That one looks jewish. And that one's a coon! Who let all this riff raff into my room?", "There's one smoking a joint. And another with spots! If I had my way I'd have all of ya shot."))
					if(prob(20)) emote(pick("giggle", "laugh"))
					if(prob(10)) src << sound(pick('rustle1.ogg', 'rustle2.ogg', 'rustle3.ogg', 'rustle4.ogg', 'rustle5.ogg', 'meteorimpact.ogg', 'squishy.ogg', 'taser.ogg', 'ep90_4.ogg', 'ep90_3.ogg', 'armbomb.ogg', 'bewareilive.ogg', 'ihunger.ogg', 'hiss1.ogg', 'hiss2.ogg', 'hiss3.ogg', 'hiss4.ogg', 'hiss5.ogg', 'runcoward.ogg','redalert3.ogg'))
					if(prob(5))
						src << sound(pick('bjustice.ogg', 'bfreeze.ogg'))
