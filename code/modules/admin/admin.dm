...
//		var/list/alljobs = get_all_jobs()
	...
	if (href_list["newban"])
		...
		//			M.achievement_give("Banned!", 13, -100)
	...
	if (href_list["remove"])
		if ((src.rank in list( "Administrator", "Primary Administrator", "Shit Guy", "Coder", "Host"  )))
			...
			if(t && isgoon(t))
				log_admin("[key_name(usr)] removed [t] from the dicklist.")
				message_admins("\blue [key_name_admin(usr)] removed [t] from the dicklist.")
				remove_goon(t)
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if (href_list["adminplayeropts"])
		...
		if (ismob(M) && M.client)
			...
			if(!istype(M, /mob/new_player))
				...
				foo += text("<A HREF='?src=\ref[src];sendtopuzzlechamber=\ref[M]'>Puzzle Area</A> | ")
	...
	/*				if("shockwave")
				...
				if("goblob")
					...
					var/turf/T = pick(blobstart)
					spawn(0)
						empulse(T, rand(1, 6), rand(1,8), nolog=1)
				if("aliens")
					if(aliens_allowed)
						alien_infestation()
				...
				if("virus")
					if(alert("Do you want this to be a random disease or do you have something in mind?",,"Random","Choose")=="Random")
						...
						var/list/viruses = list("fake gbs","gbs","magnitis","wizarditis","beesease","brain rot","cold","retrovirus","flu","swineflu","birdflu","pierrot's throat","rhumba beat", "tvirus", "mochashakah", "xmas_cheer", "plague", "inhalational_anthrax", "cosby", "gay", "ebola")
				...
				if("sexyman")
					if (src.rank in list("Shit Guy", "Coder", "Host"))
						...
						for(var/mob/living/carbon/human/B in world)
							B.gay = "1"
						message_admins("[key_name_admin(usr)] activated sexyman mode")
					else
						alert("You cannot perform this action. You must be of a higher administrative rank!")
						return
				if("dorf")
		...
		for(var/mob/M in mobs)
			if(M.ckey)
				...
				if(istype(M, /mob/living/carbon/alien))
					dat += "<td>Alien</td>"
				dat += {"<td>[(M.client ? "[(M.client.goon ? "<font color=orange>" : "<font>")][M.client]</font>" : "No client")]</td>
...
/obj/admins/proc/goons()
	var/dat = "<HR><B>Dicks</B><HR><table cellspacing=5><tr><th>Key</th><th>SD Username</th></tr>"
...
<A href='?src=\ref[src];secretsfun=goblob'>Spawn EMP</A><BR>
...
<A href='?src=\ref[src];secretsfun=sexyman'>Sexyman Mode</A><BR>
...
/obj/admins/proc/togglegoonsay()
	set category = "Server"
	set desc = "Toggle dis bitch"
	set name = "Toggle cocksay"
	goonsay_allowed = !( goonsay_allowed )
	if (goonsay_allowed)
		world << "<B>The COCKSAY channel has been enabled.</B>"
	else
		world << "<B>The COCKSAY channel has been disabled.</B>"
	log_admin("[key_name(usr)] toggled Cocksaysay to [goonsay_allowed].")
	message_admins("[key_name_admin(usr)] toggled COCKSAY [goonsay_allowed ? "on" : "off"]", 1)
