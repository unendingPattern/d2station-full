...
//		var/list/alljobs = get_all_jobs()
	...
	if(href_list["jobban3"])
		if (src.rank in list( "High Administrator", "Normal Administrator", "Primary Administrator", "Shit Guy", "Coder", "Host"  ))
	...
	if (href_list["boot2"])
		if ((src.rank in list( "Moderator", "Normal Administrator", "High Administrator", "Primary Administrator", "Shit Guy", "Coder", "Host"  )))
	...
	if (href_list["newban"])
		if ((src.rank in list( "Normal Administrator", "High Administrator", "Primary Administrator", "Shit Guy", "Coder", "Host"  )))
			...
			switch(alert("Temporary Ban?",,"Yes","No"))
				if("Yes")
					...
					M << "\red To try to resolve this matter head to http://forums.d2k5.com/space-station-13/"
	...
	if (href_list["remove"])
		if ((src.rank in list( "High Administrator", "Primary Administrator", "Shit Guy", "Coder", "Host"  )))
	...
	if (href_list["sendtoprison"])
		if ((src.rank in list( "Moderator", "High Administrator", "Normal Administrator", "Primary Administrator", "Shit Guy", "Coder", "Host"  )))
	...
	if (href_list["makeai"]) //Yes, im fucking lazy, so what? it works ... hopefully
		if ((src.rank in list( "Primary Administrator", "Coder", "Host", "High Administrator", "Shit Guy"  )))
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if (href_list["prom_demot"])
		if ((src.rank in list("Primary Administrator", "Shit Guy", "Coder", "Host"  )))
			...
			//host
				...
				<A href='?src=\ref[src];chgadlvl=Normal Administrator;client4ad=\ref[C]'>SA</A><BR>
			...
			//PA
				dat += {"
				<A href='?src=\ref[src];chgadlvl=Administrator;client4ad=\ref[C]'>A</A><BR>				<A href='?src=\ref[src];chgadlvl=Normal Administrator;client4ad=\ref[C]'>SA</A><BR>
				<A href='?src=\ref[src];chgadlvl=Moderator;client4ad=\ref[C]'>M</A><BR>
				<A href='?src=\ref[src];chgadlvl=Goat Fart;client4ad=\ref[C]'>Goat Fart</A><BR>
				<A href='?src=\ref[src];chgadlvl=Remove;client4ad=\ref[C]'>Remove Admin</A><BR>"}
			else
	...
	if (href_list["secretsadmin"])
		if ((src.rank in list( "Moderator", "High Administrator", "Primary Administrator", "Shit Guy", "Coder", "Host"  )))
			var/ok = 0
...
/obj/admins/proc/Game()
	...
	switch(src.rank)
		if("Moderator")
			lvl = 1
		if("Normal Administrator")
			lvl = 2
		if("High Administrator")
			lvl = 3
...
/obj/admins/proc/vmode()
	...
	//var/confirm = alert("What vote would you like to start?", "Vote", "Restart", "Cancel")
	if(confirm == "Cancel")
		return
	if(confirm == "Restart")
		vote.mode = 0
...
/obj/admins/proc/votekill()
	...
	if(vote.voting == 0)
		...
		return
	world << "\red <b>*** Voting aborted by [usr.client.stealth ? "High Administrator" : usr.key].</b>"
...
/obj/admins/proc/restart()
	...
	if(confirm == "Cancel")
		return
	if(confirm == "Yes")
		world << "\red <b>Restarting world!</b> \blue Initiated by [usr.client.stealth ? "High Administrator" : usr.key]!"
...
/obj/admins/proc/announce()
	...
	if (message)
		...
		world << "\blue <b>[usr.client.stealth ? "High Administrator" : usr.key] Announces:</b>\n \t [message]"
...
/obj/admins/proc/immreboot()
	...
	if( alert("Reboot server?",,"Yes","No") == "No")
		return
	world << "\red <b>Rebooting world!</b> \blue Initiated by [usr.client.stealth ? "High Administrator" : usr.key]!"
