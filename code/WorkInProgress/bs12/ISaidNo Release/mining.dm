...
/datum/controller/gameticker/proc/setup_isno_mining()
	// Mining Setup
	...
	for(var/obj/landmark/mining/O in T4veins)
		...
		if (O.veinsetup) continue
		if (prob(95))
			O.ore1 = pick("erebite","cerenkite","Elerium")
			T.ore1 = O.ore1
			if (T.ore1 == "erebite" || T.ore1 == "Elerium") T.explosive = 1
			T.icon_state = "[T.ore1][rand(1,3)]"
			T.ore2 = O.ore2
			var/amtlower = 1
			var/amtupper = 3
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		if(istype(W,/obj/item/weapon/drill) || istype(W,/obj/item/weapon/pickaxe/))
			if (istype(W,/obj/item/weapon/pickaxe/powered))
				if (W:status)
					playsound(user.loc, 'trayloading.wav', 100, 1)
					playsound(user.loc, 'welder.ogg', 100, 1)
			...
			if (istype(W,/obj/item/weapon/drill))
				playsound(user.loc, 'bonedrill.ogg', 75, 1)
			else playsound(user.loc, 'trayloading.wav', 100, 1)
			...
			playsound(user.loc, 'smash.ogg', 100, 1)
	...
	proc/destroy_asteroid(var/plasmacutter = 1)
		...
		for(makeores = src.amount, makeores > 0, makeores--)
			...
			switch(src.ore1)
				...
				if("Elerium") new/obj/item/weapon/ore/Elerium(src)
	...
	attack_self(var/mob/user as mob)
		if (src.charges)
			if (!src.status)
				...
				src.overlays += image('weapons.dmi', "pp-glow")
				item_state = "powerpick"
				src.status = 1
				src.minelevel = 2
			else
...
/obj/item/weapon/plastique/breachcharge/mining
	name = "Mining Explosive"
	desc = "It is set to detonate in 5 seconds."
	flags = ONBELT
	w_class = 1
	var/emagged = 0
	var/expl_heavy = 1	//Strumpetplaya - Commented out as it is currently unsupported
	var/expl_light = 6
	var/expl_flash = 10
		...
		if (user.equipped() == src)
			...
			if (user.mutations & 16)
				...
				if (istype(target, /turf/simulated/wall/asteroid/) && !src.emagged)
					...
					spawn( timer*10 )
						//boom()	Strumpetplaya - Commented out as it is currently unsupported
						explosion(loc, -1, expl_heavy, expl_light, expl_flash)
						if(target)
...
/obj/item/weapon/plastique/breachcharge/mining/light
...
/obj/item/weapon/oreprospector
	...
	attack_self(var/mob/user as mob)
		...
		for (var/turf/simulated/wall/asteroid/T in range(user,6))
			...
			if (T.ore1 == "erebite" || T.ore1 == "cerenkite" || T.ore1 == "Elerium" || T.event) unusual++
			switch (T.event)
				if("worm") unusual++
				if("volatile") unusual++
...
/mob/proc/mining_radcheck(var/mob/user as mob)
	if (istype(user, /mob/living/carbon/human/))	//Strumpetplaya - Commented out as it is currently unsupported
		if (istype(user:wear_suit, /obj/item/clothing/suit/space/rig)||istype(user:wear_suit, /obj/item/clothing/suit/radiation)) return 1
		else return 0
	else return 1

/obj/item/weapon/ore/Elerium
	name = "Elerium"
	desc = "A piece of Elerium, a fuel source similar to plasma."
	icon_state = "Elerium"
